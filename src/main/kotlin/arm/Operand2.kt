...
data class LSLRegOperand2(val rm: Reg, val rs: Reg) : Operand2() {
  override val code = "${rm.code} LSL ${rs.code}"
}
data class LSRRegOperand2(val rm: Reg, val rs: Reg) : Operand2() {
  override val code = "${rm.code} LSR ${rs.code}"
}
data class ASRRegOperand2(val rm: Reg, val rs: Reg) : Operand2() {
  override val code = "${rm.code} ASR ${rs.code}"
}
data class RORRegOperand2(val rm: Reg, val rs: Reg) : Operand2() {
  override val code = "${rm.code} ROR ${rs.code}"
}
data class RRXOperand2(val rm: Reg) : Operand2() {
